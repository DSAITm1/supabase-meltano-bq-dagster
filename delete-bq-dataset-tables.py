...
def delete_specific_tables(client, project_id, dataset_name, table_prefixes=['dim', 'fact'], dry_run=False):
    """Delete specific tables from the dataset based on prefixes"""
    try:
        ...
        all_tables = list(client.list_tables(dataset))
        if not all_tables:
        ...
        for table in all_tables:
            if any(table.table_id.startswith(prefix) for prefix in table_prefixes):
                tables_to_delete.append(table)
            else:
                other_tables.append(table)
        
        print(f"📊 Dataset contains {len(all_tables)} total tables:")
        
        if tables_to_delete:
            print(f"\n🎯 Tables to DELETE (starting with {', '.join(table_prefixes)}):")
            for table in tables_to_delete:
                print(f"   ❌ {table.table_id}")
        
        if other_tables:
            print(f"\n✅ Tables to KEEP (not matching prefixes):")
            for table in other_tables:
                print(f"   ✅ {table.table_id}")
        
        if not tables_to_delete:
            print(f"\n📋 No tables found starting with {', '.join(table_prefixes)}")
            return True
        
        if dry_run:
            print(f"\n🧪 DRY RUN: Would delete {len(tables_to_delete)} tables from '{dataset_name}'!")
            return True
        
        # Confirm deletion
        print(f"\n⚠️  WARNING: This will permanently delete {len(tables_to_delete)} tables from dataset '{dataset_name}'!")
        ...
        # Delete the specified tables
        deleted_count = 0
        for table in tables_to_delete:
            try:
                table_id = f"{project_id}.{dataset_name}.{table.table_id}"
        ...
        print(f"\n✅ Successfully deleted {deleted_count} out of {len(tables_to_delete)} tables")
        return True
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description='Delete BigQuery dataset or specific tables')
    ...
    parser.add_argument('--delete-tables', action='store_true',
                       help='Delete specific tables instead of entire dataset')
    parser.add_argument('--table-prefixes', nargs='+', default=['dim', 'fact'],
                       help='Table prefixes to delete (default: dim fact)')
    args = parser.parse_args()
    
    print("🗑️  BigQuery Dataset/Table Deletion Tool")
    print("=" * 50)
    ...
    try:
        ...
        print(f"🗂️  Target dataset: {dataset_name}")
        
        if args.delete_tables:
            print(f"🎯 Mode: Delete specific tables with prefixes: {', '.join(args.table_prefixes)}")
        else:
            print("🎯 Mode: Delete entire dataset")
        ...
        # Delete tables or entire dataset based on mode
        if args.delete_tables:
            print(f"\n🎯 Attempting to delete specific tables from dataset: {dataset_name}")
            success = delete_specific_tables(client, actual_project_id, dataset_name, 
                                           args.table_prefixes, args.dry_run)
        else:
            print(f"\n🗑️  Attempting to delete entire dataset: {dataset_name}")
